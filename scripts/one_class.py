...
# from comms import (self.roadx, self.roady)
# from comms import (self.rx, self.ry)
...
class Broadcaster_Listener:
    ...
    def callback(self,msg):
        ...
        if x==1 :
            ...
            self.trajecx, self.trajecy = prm_planning(self.sx, self.sy, self.gx, self.gy, self.px, self.py, 0.1)
            self.trajecx = np.array(self.trajecx)
            self.trajecy = np.array(self.trajecy)
            
            self.trajecx = self.trajecx[::-1]
            self.trajecy = self.trajecy[::-1]
            # np.savetxt('tr_x.csv',tr_x,delimiter=',')
            # np.savetxt('tr_y.csv',tr_y,delimiter=',')
            # print(self.trajecx)
            # self.trajecx = np.loadtxt('/home/juansantacoloma/rycs/taller4/taller4_rycsv/scripts/tr_x.csv',delimiter=',')
            # self.trajecy = np.loadtxt('/home/juansantacoloma/rycs/taller4/taller4_rycsv/scripts/tr_y.csv',delimiter=',')
